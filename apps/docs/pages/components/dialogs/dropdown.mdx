...
export const PrgrammaticDropdown = () => {
  ...
  return (
    <Components.Stack direction="row" columnGap={16} verticalAlign="center">
      <Components.Button onClick={() => setIsOpen(open => !open)}>
        {isOpen ? 'Close' : 'Open'} dropdown
      </Components.Button>
      <Components.Dropdown
        open={isOpen}
        placement="right"
        trigger={<Components.Icon source="circle-question" />}
      >
...
<PropsTable
  properties={[
    ...
    },
    {
      ...
      default: "false",
      required: false,
      description: "Programmatically open or close the dropdown. If set to `true`, the dropdown will be open when rendered. If the dropdown is open, it will force close if `open` is set to `false`.",
    },
  ]}
/>
