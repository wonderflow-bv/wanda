...
export const CustomExpandTable = () => {
  ...
  return (
    <Components.Table
      columns={columns}
      data={data.slice(0, 4)}
      expandableRowsComponent={(data) => <CodeBlock background="var(--global-background)" className="language-json">{JSON.stringify(data, null, 2)}</CodeBlock>}
    />
  );
};
...
export default () => {
  return <Table
    columns={columns}
    data={data}
    expandableRowsComponent={(subRow) => (
      <div>
