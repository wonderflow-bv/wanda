...
- It is best practice not to use a popover if there are two options to choose
  from. In this case, use [radio buttons](/documentation/components/inputs/selection-controls) instead.
...
Even though Popovers and [Selects](/documentation/components/inputs/select) share similar functionalities, they are actually behaviourally unique and satisfy different use cases:
...
There are [different preset](/documentation/components/dialogs/popover#prop-placement) that you can give to your component, but the `auto`, `auto-start`, `auto-end` are really convenient.

### Elevation

The Popover has a [default elevation](/documentation/usage/design/foundation/elevations#default-elevations) of 2
